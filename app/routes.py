...
from app.models import Sites, Columns
...
def index():
    ...
    return render_template('index.html', title='Home', data1=data1, data2=data2, data3=data3, fonts=columns)
...
def modify_site(url):
    ...
    form = UpdateForm(column=int(site.column_id))
    ...
    if form.validate_on_submit():
        if form.submit.data:
            ...
            site.column_id = form.column.data
            db.session.commit()
...
def add_site():
    form = CreateForm()
    if form.validate_on_submit():
        site = Sites(site_url=form.url.data, site_colour=form.colour.data, column_id=form.column.data)
        db.session.add(site)
        db.session.commit()
